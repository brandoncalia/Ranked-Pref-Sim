...
In two dimensional space, we choose to distribute voters such that each x and y coordinate is drawn from a normal distribution, $\mu = 0, \sigma = .33$. Candidate coordinates are drawn from a normal distribution, $\mu = 0, \sigma = .33$. We found this slight variation to better account for slight imperfections we observed of the spatial model. (In short, distributing candidates too close to the center almost guaranteed that a plurality winner would just be the candidate farthest from the center.) However, additional politcal science knowledge on the distribution of voter beliefs would be useful (rather, necessary) to improve these choices. 
